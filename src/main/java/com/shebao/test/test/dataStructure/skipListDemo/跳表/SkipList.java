...
public class SkipList<T> {
    ...
    public void add(SkipNode<T> node) {
        ...
        while (!stack.isEmpty()) {
            ...
            SkipNode<T> nodeTeam = new SkipNode<T>(node.key, node.value);//节点需要重新创建
    ...
    public void printf(){
        int level = highLevel;
        SkipNode skipNode = headNode;
        for (int i = level; i > 0; i--) {
            System.out.print("第"+i+"层：");
            SkipNode reNode = skipNode;
            while (reNode.right != null){
                String s = " ";
                for (int j = 0; j < reNode.right.key; j++) {
                    s+=" ";
                }
                System.out.print(s+reNode.right.value);
                reNode = reNode.right;
            }
            System.out.println();
...
class SkipNode<T> {
    int key;
    T value;
    SkipNode<T> right, down;//右下个方向的指针
